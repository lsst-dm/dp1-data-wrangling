...
async def export_datasets(
    ...
    datastore_path = Path(output_directory).joinpath(
        f"{dataset_type.name}.datastore.parquet"
    )
    async with asyncio.TaskGroup() as tg:
        # In the background, read back in the list of IDs from the datasets we
        # exported above, and insert them to a queue.
        datastore_lookup_queue = ProduceConsumeQueue[DatasetId](4)
        tg.create_task(
            asyncio.to_thread(
                _read_back_dataset_ids, dataset_path, datastore_lookup_queue
            )
        )
        # In the background, with multiple lookups in parallel, look up
        # datastore records associated with the datasets.
        limiter = TaskLimiter(tg, 8)
        async for dataset_ids in datastore_lookup_queue.consume_iter():
            await limiter.create_task(
                _fetch_datastore_records(butler_pool, dataset_ids)
            )
...
# HAVE _fetch_datastore_records INSERT THESE TO A QUEUE
type ButlerDatastoreRecords = Mapping[]

async def _fetch_datastore_records(
    butler_pool: ButlerPool, dataset_ids: Iterable[DatasetId]
) -> None:
    async with butler_pool.get_butler() as butler:
